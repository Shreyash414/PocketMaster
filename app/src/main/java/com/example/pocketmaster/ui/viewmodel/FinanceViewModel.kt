...
class FinanceViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun getTransactionsByType(type: TransactionTypes) =
        repository.getTransactionByType(type)

    fun getCategoriesByType(type: TransactionTypes) =
        repository.getCategoryByType(type)

    fun getCategoryTotals(type: TransactionTypes, startDate: Long = 0) =
        repository.getCategoryTotals(type, startDate)

    suspend fun getTotalByType(type: TransactionTypes) =
